# Milestone v0.1.4 Requirements Archive

**Shipped:** 2026-04-27
**Milestone:** Operator Surface
**Status:** ✅ ALL REQUIREMENTS COMPLETE (41/41)
...
- [x] **NAV-01**: `/arcanon:list` — concise project overview output. Format:
...
- [x] **NAV-04**: `/arcanon:diff <scanA> <scanB>` — compare two scan_versions. Accepts:
...
- [x] **HELP-01**: Every command markdown file (currently 9: `map`, `impact`, `drift`, `sync`, `login`, `status`, `export`, `update`, `verify`) gets a `## Help` section with usage block + 2-3 examples. Section is the source of truth — no separate help text file.

- [x] **HELP-02**: A small bash detector wrapper (or `${CLAUDE_PLUGIN_ROOT}/lib/help.sh` helper) checks `$ARGUMENTS` for `--help` / `-h` / `help` and prints the `## Help` section content via `awk` extraction. Each command's run section calls the helper before its real logic.

- [x] **HELP-03**: Pre-existing `commands/update.md:21` `claude plugin update --help` reference (upstream Claude Code host CLI flag) is preserved as-is — VER grep should refine to `/arcanon:.*--help` or whitelist that one line.

- [x] **HELP-04**: bats test — iterate every command file under `plugins/arcanon/commands/*.md`, invoke `<cmd> --help`, assert non-empty output and 0 exit code. Fails if any command lacks the `## Help` section.
...
- [x] **FRESH-01**: `/arcanon:status` output now includes a `Latest scan: <date> (NN% high-confidence)` line — same data the SessionStart banner already carries.

- [x] **FRESH-02**: New per-repo signal: `git log <last_scan_sha>..HEAD --oneline | wc -l` per tracked repo. Reports count of repos with un-scanned commits since last scan, and the number of new commits in each.
...
- [x] **FRESH-03**: New worker HTTP endpoint `GET /api/scan-freshness?project=<root>` returns `{last_scan_iso, last_scan_age_seconds, scan_quality_pct, repos: [{name, path, last_scanned_sha, new_commits}]}`. Mirrors `/api/scan-quality` and `/api/version` patterns.

- [x] **FRESH-04**: `/arcanon:status` calls the new endpoint via `worker-client.sh` `worker_call` helper.

- [x] **FRESH-05**: bats test — fixture with mock git repo + seeded scan_version → assert status output contains expected freshness line.
...
- [x] **CORRECT-01**: Migration `017_scan_overrides.js` creates `scan_overrides` table:
...
- [x] **CORRECT-03**: Scan pipeline reads `scan_overrides` BEFORE `endScan` and applies pending overrides to the persisted findings. Override is marked `applied_in_scan_version_id` on apply. Already-applied overrides skipped on subsequent scans.
...
- [x] **INT-01**: New config flag `hub.evidence_mode: "full" | "hash-only" | "none"`. Default `"full"` for back-compat. In `"hash-only"` mode, the hub upload payload's `evidence` field is replaced with `{ "hash": "<sha256>", "start_line": N, "end_line": M }` instead of the raw evidence string.
...
- [x] **INT-03**: Hub Payload schema (existing `worker/hub-sync/payload.js`) extended to accept hashed evidence form. Backward-compatible: payload remains valid v1.1; hash-only is a new optional shape on the `evidence` field.
