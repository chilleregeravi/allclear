...
- [x] **LOG-03**: All error log calls include err.stack alongside err.message — stack traces visible in structured log output for post-mortem debugging
...
- [x] **ERR-01**: HTTP route error handlers log errors to the structured logger (not just response body) — all catch blocks in http.js that return 500 also call logger.error with stack trace
