...
setup() {
  ...
  unset _ARCANON_CONFIG_LOADED
  unset ARCANON_CONFIG_FILE
  unset ARCANON_CONFIG_LINKED_REPOS
...
@test "config.sh loads siblings from arcanon.config.json" {
  cd "$FIXTURE_DIR"
  source "$LIB_CONFIG"
  [ "${#ARCANON_CONFIG_LINKED_REPOS[@]}" -eq 3 ]
  [ "${ARCANON_CONFIG_LINKED_REPOS[0]}" = "../api" ]
  [ "${ARCANON_CONFIG_LINKED_REPOS[1]}" = "../ui" ]
  [ "${ARCANON_CONFIG_LINKED_REPOS[2]}" = "/opt/repos/sdk" ]
}

@test "config.sh returns empty siblings when no config file" {
  cd "$BATS_TEST_TMPDIR"
  source "$LIB_CONFIG"
  [ "${#ARCANON_CONFIG_LINKED_REPOS[@]}" -eq 0 ]
}
...
@test "config.sh respects ARCANON_CONFIG_FILE override" {
  # Source from a different dir but point config at fixture
  cd "$BATS_TEST_TMPDIR"
  export ARCANON_CONFIG_FILE="$FIXTURE_DIR/arcanon.config.json"
  source "$LIB_CONFIG"
  [ "${#ARCANON_CONFIG_LINKED_REPOS[@]}" -eq 3 ]
  [ "${ARCANON_CONFIG_LINKED_REPOS[0]}" = "../api" ]
}
...
@test "config.sh guard prevents double loading" {
  cd "$FIXTURE_DIR"
  source "$LIB_CONFIG"
  local first_count="${#ARCANON_CONFIG_LINKED_REPOS[@]}"
  # Source again — should return early, no error
  source "$LIB_CONFIG"
  [ "${#ARCANON_CONFIG_LINKED_REPOS[@]}" -eq "$first_count" ]
}
...
@test "ARCANON_LINT_THROTTLE overrides default 30s" {
  run env ARCANON_LINT_THROTTLE=10 bash "$FIXTURE_DIR/mock-lint.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"throttle=10"* ]]
}

@test "invalid ARCANON_LINT_THROTTLE falls back to 30" {
  run env ARCANON_LINT_THROTTLE=abc bash "$FIXTURE_DIR/mock-lint.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"throttle=30"* ]]
}

@test "unset ARCANON_LINT_THROTTLE uses default 30" {
  run env -u ARCANON_LINT_THROTTLE bash "$FIXTURE_DIR/mock-lint.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"throttle=30"* ]]
}
...
@test "ARCANON_EXTRA_BLOCKED blocks matching file" {
  run env ARCANON_EXTRA_BLOCKED="*.bak:*.tmp" bash "$FIXTURE_DIR/mock-guard.sh" "foo.bak"
  [ "$status" -eq 2 ]
  [[ "$output" == *"blocked:foo.bak"* ]]
}

@test "ARCANON_EXTRA_BLOCKED allows non-matching file" {
  run env ARCANON_EXTRA_BLOCKED="*.bak" bash "$FIXTURE_DIR/mock-guard.sh" "foo.ts"
  [ "$status" -eq 0 ]
  [[ "$output" == *"allowed:foo.ts"* ]]
}

@test "multiple ARCANON_EXTRA_BLOCKED patterns all checked" {
  run env ARCANON_EXTRA_BLOCKED="*.bak:*.tmp:*.old" bash "$FIXTURE_DIR/mock-guard.sh" "data.tmp"
  [ "$status" -eq 2 ]
}
