...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'allclear-test-'));
}
...
test('loadFromConfig returns [] when allclear.config.json is missing', () => {
...
test('loadFromConfig returns [] when linked-repos key is absent', () => {
  ...
  try {
    fs.writeFileSync(
      path.join(projectRoot, 'allclear.config.json'),
...
test('discoverNew excludes current project dir', () => {
  ...
  try {
    ...
    assert.ok(!paths.includes(projectRoot), 'should not include projectRoot itself');
  } finally {
    cleanup(parentDir);
  }
});
...
test('discoverNew excludes paths already in existingPaths', () => {
  ...
  try {
    ...
    const paths = result.map(r => r.path);
    assert.ok(!paths.includes(existingRepo), 'should not include existing path');
  } finally {
    cleanup(parentDir);
  }
});
...
test('discoverNew returns only dirs containing a manifest file', () => {
  ...
  fs.mkdirSync(goRepo);
  fs.writeFileSync(path.join(goRepo, 'go.mod'), 'module example.com/go-repo\n\ngo 1.21\n');
  ...
  try {
    ...
    assert.ok(!paths.includes(emptyDir), 'no-manifest dir should not be discovered');
    assert.ok(!paths.includes(projectRoot), 'projectRoot should not be discovered');
...
test('saveConfirmed round-trip: write then read preserves linked-repos and other keys', () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(projectRoot, 'allclear.config.json'),
      JSON.stringify({ 'impact-map': { history: true }, 'other-key': 'value' }, null, 2)
    );
    ...
    const saved = JSON.parse(
      fs.readFileSync(path.join(projectRoot, 'allclear.config.json'), 'utf8')
    );
    assert.deepEqual(saved['linked-repos'], paths, 'linked-repos persisted');
    assert.deepEqual(saved['impact-map'], { history: true }, 'impact-map preserved');
...
test('saveConfirmed creates allclear.config.json when absent', () => {
  const projectRoot = makeTempDir();
  try {
...
test('formatRepoList with mixed new/confirmed repos produces correct output with [NEW] markers', () => {
  const repos = [
    { path: '/abs/confirmed-repo', name: 'confirmed-repo', source: 'config', isNew: false },
    { path: '/abs/new-repo', name: 'new-repo', source: 'discovered', isNew: true },
  ];
  const output = formatRepoList(repos);
  assert.ok(output.includes('Found 2 repos (1 new)'), 'summary line correct');
  assert.ok(output.includes('[confirmed] /abs/confirmed-repo (confirmed-repo)'), 'confirmed line present');
  assert.ok(output.includes('[NEW] /abs/new-repo (new-repo)'), '[NEW] line present');
  assert.ok(output.includes('<-- newly discovered'), 'newly discovered marker present');
